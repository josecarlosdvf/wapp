...
export const getBodyMessage = (msg: proto.IWebMessageInfo): string | null => {
  ...
  try {
    ...
    let isLinkMessage = msg.message.extendedTextMessage?.description;
    let linkMessage = JSON.stringify(msg.message.extendedTextMessage);
    const types = {
      conversation: msg?.message?.conversation,
      imageMessage: msg.message?.imageMessage?.caption,
      videoMessage: msg.message.videoMessage?.caption,
      extendedTextMessage: isLinkMessage ? linkMessage : msg.message.extendedTextMessage?.text,
      ...
      buttonsMessage: msg.message?.buttonsMessage?.contentText || getBodyButton(msg) || msg.message.listResponseMessage?.singleSelectReply?.selectedRowId,
      ...
      documentMessage: msg.message?.documentMessage?.title || msg.message?.documentWithCaptionMessage?.message?.documentMessage?.title,
      audioMessage: "Áudio",
      listMessage: msg.message?.listMessage?.description || getBodyButton(msg) || msg.message.listResponseMessage?.title,
...
const downloadMedia = async (msg: proto.IWebMessageInfo) => {
  ...
  const messageType = msg.message?.documentMessage || msg.message?.documentWithCaptionMessage?.message?.documentMessage
  ...
  try {
    wbot.ev.on("messages.upsert", async (messageUpsert: ImessageUpsert) => {
      ...
      messages.forEach(async (message: proto.IWebMessageInfo) => {
        ...
        if (!messageExists) {
          if (message.message?.extendedTextMessage?.contextInfo?.externalAdReply?.title) {
            //link campanha com legenda
            let messageCustom = {
                key: message?.key,
                ...
                message: {
                  extendedTextMessage: {
                    text: message.message?.extendedTextMessage?.contextInfo?.externalAdReply?.sourceUrl,
                    matchedText: message.message?.extendedTextMessage?.contextInfo?.externalAdReply?.sourceUrl,
                    canonicalUrl: message.message?.extendedTextMessage?.contextInfo?.externalAdReply?.sourceUrl,
                    description: ' ',
                    thumbnailUrl: message.message?.extendedTextMessage?.contextInfo?.externalAdReply?.thumbnailUrl,
                    title: message.message?.extendedTextMessage?.contextInfo?.externalAdReply?.title,    
                    inviteLinkGroupTypeV2: 0
                  },
                }
              };
            
            await handleMessage(messageCustom, wbot, companyId);
            await verifyRecentCampaign(messageCustom, companyId);
            await verifyCampaignMessageAndCloseTicket(messageCustom, companyId);
        }else
          if (message.message?.documentWithCaptionMessage) {
            let messageCustom = {
              ...
              messageTimestamp: message?.messageTimestamp,
              pushName: message?.pushName,
              message: message.message?.documentWithCaptionMessage?.message
            };
            let messageTextCustom = {
              key: message?.key,
              messageTimestamp: message?.messageTimestamp,
              pushName: message?.pushName,
              message: {
                conversation: message.message?.documentWithCaptionMessage?.message?.documentMessage?.caption,
              }
            };
            ...
            await handleMessage(messageTextCustom, wbot, companyId);
            await verifyRecentCampaign(messageTextCustom, companyId);
            await verifyCampaignMessageAndCloseTicket(messageTextCustom, companyId);
            ...
            let messageTextCustom = {
              ...
              pushName: message?.pushName,
              message: {
                conversation: message.message?.imageMessage?.caption
              }
            };
            await handleMessage(messageTextCustom, wbot, companyId);
            await verifyRecentCampaign(messageTextCustom, companyId);
