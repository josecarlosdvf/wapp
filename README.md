...
```bash
cd ~
git clone git@github.com:w3nder/whatspainel.git
```

Crie um arquivo .env de backend e preencha com as informações correta:

```bash
cp whatspainel/backend/.env.example whatspainel/backend/.env
nano whatspainel/backend/.env
```
